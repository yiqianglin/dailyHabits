<template>
  <div v-cloak id="clinical_recruitment" :class="{'category-drug': queryDrugName}" class="container">
    <tmenu :stag="stag" :name="queryDisease ? `${queryDisease}-临床试验` : '临床试验信息'" @callback="tmenuCb" />
    ...
    <div v-else class="cagetory-diseases">
      ...
      <div class="doc-selector-wrp">
        <div class="doc-selector-inner-wrp">
          <div class="doc-selector doc-selector-disease" @click="docSelectorClick(1)">
            <div class="doc-selector-inner">{{ tradeNamePickerData.slot[0].currentValue | pickerShow('选择治疗方式') }}</div>
            <a href="javascript:;" class="btn-link" />
          </div>
          <div class="doc-selector doc-selector-city" @click="docSelectorClick(2)">
            <div class="doc-selector-inner">{{ cityPickerData.slot[0].currentValue | pickerShow('选择试验城市') }}</div>
            <a href="javascript:;" class="btn-link" />
          </div>
        </div>
      </div>
      <div v-if="!feedsContent.isEmpty" id="content-wrp" class="content-wrp">
        <div class="remark">
          声明：以下信息均转载自国家药品监督管理局药品审评中心网站，具体以实际情况为准
        </div>
        <!-- <paper :paperlist="feedsContent.data" :loaded="feedsContent.loaded" :rkey="'sss'"></paper>
      ...
      <div v-if="feedsContent.isEmpty" class="empty-remark">暂时没有符合您要求的临床试验</div>
      <div v-if="feedsContent.isEmpty" class="other-docs-wrp">
        <p class="other-p">其他相关试验</p>
        <div class="remark">
          声明：以下信息均转载自国家药品监督管理局药品审评中心网站，具体以实际情况为准
        </div>
        <div class="other-docs">
          <div id="paper3" class="paper">
            <template v-for="(item,index) in otherContent.data">
              <div v-show="item.title!='' || item.text!=''" :key="index" :class="{'no-img':!item.op_image}" class="item">
                <div class="content-detail">
                  <div class="info">
                    <div class="title no-desc">{{ item.title }}</div>
            ...
            <p v-if="!otherContent.isEmpty" class="loading">
              <span v-show="!otherContent.loaded" class="dotting">
                <i/>
                <i/>
                <i/>
            </span>{{ otherContent.loaded?'已经到底了':'正在加载更多' }}</p>
            <div v-if="otherContent.isEmpty" class="paper-empty">{{ otherContent.loaded?'暂时没有符合的临床试验':'数据加载中...' }}</div>
          </div>
        </div>
      </div>
      ...
      <!--筛选治疗方式-->
      <picker
        :is-show="tradeNamePickerIsShow"
        :data="tradeNamePickerData && tradeNamePickerData.slot" :title="tradeNamePickerData && tradeNamePickerData.name"
        @onCancel="tradeNamePickerCancel"
        @onConfirm="tradeNamePickerConfirm"
      />
      <!--筛选城市-->
      <picker
        :is-show="cityPickerIsShow"
        :data="cityPickerData && cityPickerData.slot" :title="cityPickerData && cityPickerData.name"
        @onCancel="cityPickerCancel"
        @onConfirm="cityPickerConfirm"
      />
    </div>

    <search :options="{mini: true, pass: true, show: showAppSearch, stag: stag}" @callback="searchCb" />
  </div>
</template>

<script>
import clinicalRecruitmentEspecially from './clinical_recruitment_especially'
export default clinicalRecruitmentEspecially
</script>

<style lang="scss" scoped>
@import "./clinical_recruitment_especially.scss";
</style>
