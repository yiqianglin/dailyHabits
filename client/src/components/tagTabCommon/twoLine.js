...
export default {
  ...
  mounted () {
    this.$nextTick(() => {
      var $ul = $(`${this.parentDom} .papertag .tab-wrap`)[0]
      if ($ul) {
    ...
    this.$nextTick(() => {
      this.paperTagHeight.noExpand = $(`${this.parentDom} .papertag`).height()
    })
    this.bind()
  },
  methods: {
    ...
    scrollToCurrentTag () {
      let $tagUl = $(this.parentDom).find('.papertag').find('.items-wrp')
      ...
      if (this.activeIndex < 0) {
        ...
        this.$nextTick(() => {
          $tagUl.scrollLeft($tagUl.scrollLeft() + $tagUl.find('.item').eq(this.activeIndex).offset().left - 30)
        })
      }
    },
    showPaperTag () {
      if (this.parentDom && $(this.parentDom).length) {
        ...
        // console.log('heightNeedMinus:', heightNeedMinus)
